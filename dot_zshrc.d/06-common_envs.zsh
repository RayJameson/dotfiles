...
export HISTORY_IGNORE="(exit|clear|pwd|ls|cd)*"
export FZF_TMUX_OPTS="-p80%,60%"
export FZF_CTRL_T_COMMAND="$FZF_DEFAULT_COMMAND -t=f"
export FZF_ALT_C_COMMAND="$FZF_DEFAULT_COMMAND -t=d"
export FZF_PROMPT_SYMBOL=$([[ -n $NERD_FONT ]] && echo "❯" ||  echo ">")
export FZF_POINTER=$([[ -n $NERD_FONT ]] && echo "" ||  echo ">")
export FZF_MARKER=$([[ -n $NERD_FONT ]] && echo "⇒" ||  echo "=>")
export FZF_DEFAULT_OPTS="--ansi --bind ctrl-u:preview-up,ctrl-d:preview-down --bind ctrl-j:down,ctrl-k:up --no-multi --prompt '$FZF_PROMPT_SYMBOL ' --marker '$FZF_MARKER' --pointer '$FZF_POINTER' --reverse --color=dark
  --color=fg:-1,bg:-1,hl:#5fff87,fg+:-1,bg+:-1,hl+:#ffaf5f
  --color=info:#af87ff,prompt:#5fff87,pointer:#ff87d7,marker:#ff87d7,spinner:#ff87d7 --gutter=' '"
