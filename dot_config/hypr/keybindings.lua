...
hl.bind(K.SUPER + K.SHIFT + K.Return, hl.dsp.exec_cmd("[float; group deny; center; size (monitor_w*0.75) (monitor_h*0.75);] " .. terminal .. " --float"))
...
hl.bind(K.SUPER + K.SHIFT + K.CTRL + K.Q, hl.dsp.exec_cmd('hyprctl activewindow | grep -oP \'(?<=pid: )\\d+\' | xargs kill -9'))
...
hl.bind(K.XF86AudioRaiseVolume, hl.dsp.exec_cmd("wpctl set-volume -l 1 @DEFAULT_AUDIO_SINK@ 5%+"), { repeating = true, locked = true })
hl.bind(K.XF86AudioLowerVolume, hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-"), { repeating = true, locked = true })
hl.bind(K.XF86AudioMute, hl.dsp.exec_cmd("wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle"), { repeating = true, locked = true })
hl.bind(K.XF86AudioMicMute, hl.dsp.exec_cmd("wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle"), { repeating = true, locked = true })
...
hl.bind(K.SUPER + K.SHIFT + K.mouse_down, hl.dsp.exec_cmd('hyprctl -q keyword cursor:zoom_factor $(hyprctl getoption cursor:zoom_factor | awk \'/^float.*/ {print $2 * 1.2}\')'))
hl.bind(K.SUPER + K.SHIFT + K.mouse_up, hl.dsp.exec_cmd('hyprctl -q keyword cursor:zoom_factor $(hyprctl getoption cursor:zoom_factor | awk \'/^float.*/ {print $2 * 0.8}\')'))
